...
# Set up / libraries
rm(list=ls())
setwd("/Users/maxwelllee12/Desktop/HackNYU/hack-nyu")
getwd()
...
df$distance <- haversine_distance(input_latitude, input_longitude, df$Latitude, df$Longitude)
...
tracts_nyc <- get_acs(
  geography = "tract",
  variables = "B01001_001",  # total population (just a dummy var to get tracts)
  state = "NY",
...
# 0.5-mile buffers around each relevant station
buffers_2263 <- stations_2263 %>%
  st_buffer(dist = half_mile_ft)

# Tracts that intersect each station's 0.5-mile buffer (full or partial)
...
ggplot() +
  ...
  # Filled buffer radius = MTA color
  geom_sf(data = buffers_4326,
          aes(fill = primary_line, color = primary_line),
          alpha = 0.30, linewidth = 0.4) +
  
  # Station dots: colored by MTA color + shaped by line (unique per line)
  ...
  # Shape scale: ensures lines with same color get DIFFERENT shapes
...
# Station info: drop geometry, keep stop name, line, and coords
stations_info <- stations_sf %>%
  st_drop_geometry() %>%
  select(stop_name, primary_line, Longitude, Latitude)

# Join station averages with station coords + line
...
# JSON output
stop_rent_json <- toJSON(stop_rent_list, pretty = TRUE, auto_unbox = TRUE)
cat(stop_rent_json)
